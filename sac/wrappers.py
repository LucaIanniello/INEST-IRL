...
class GridCoverageClass():

    def __init__(self, coverage_grid_size, coverage_threshold):
    ...
    def save_and_plot_coverage(self, exp_dir, index_seed_step, rank, subtask, num_subtasks, filename='coverage_analysis.json'):
        ...
        if self._coverage_grid is not None and self._coverage_grid.ndim == 2 and rank == 0:
            try:
                ...
                plt.title(f'Coverage Grid at Step {index_seed_step}\n'
                ...
                viz_file = os.path.join(coverage_dir, f'coverage_grid_step_{index_seed_step}.png')
...
class INESTIRLLearnedVisualReward(LearnedVisualReward):
    ...
    def __init__(
        ...
        **base_kwargs,
        ...
        self.grid_coverage = GridCoverageClass(coverage_grid_size, coverage_threshold)
    ...
    def _compute_intrinsic_reward(self, emb):
        ...
        self.grid_coverage._novelty_history.append({
            'step': self.grid_coverage._total_steps,
...
class RandomEncoderIntrinsicReward(LearnedVisualReward):
    """Random encoder IRL learned visual reward with intrinsic motivation and coverage tracking."""
    def __init__(
        self,
        subtask_means,
        distance_scale,
    ...
    def _get_reward_from_image(self, random_encoder, image, flag):
        # print("WRAPPER: Computing INEST IRL reward from image.")
        image_tensor = self._to_tensor(image)
    ...
    def step(self, action, rank=0, exp_dir=None, flag="train"):
        ...
        info["env_reward"] = env_reward
        pixels = self._render_obs()
        learned_reward = self._get_reward_from_image(random_encoder, pixels, flag)
